...
def inline_array_relax_empty_shape(array: NDArray, shape: Sequence[int | str] | None) -> NDArray:
    if shape is None or any(isinstance(s, str) for s in shape):
        return array

    # unfortunately above lines do not trigger correct type-narrowing
    shape = typing.cast("Sequence[int]", shape)
...
class NDArrayType:
    def __init__(
        ...
        order,
        mask,
        data_callback: BlockAttrCallback | None = None,
    ):
        self._source = source
    ...
    def _make_array(self) -> NDArray:
        ...
        # memory mapping.
        if self._array is not None and self._data_callback is not None:
        ...
        # Currently arrays are always constructed with either inline data or a data callback
        # This should ideally be enforced by the type system
        data_callback = typing.cast("BlockAttrCallback", self._data_callback)
        ...
        if isinstance(self._source, str):
            ...
            # the same base array
            data = data_callback(_attr="cached_data")
    ...
    def __len__(self) -> int:
        if self._array is None:
            # Array being empty means that its an inline array
            return typing.cast("list[int]", self._shape)[0]
