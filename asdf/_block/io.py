...
def generate_write_header(
    ...
    **header_kwargs : dict, optional
    ...
    if header_kwargs["compression"] == b"\0\0\0\0":
        ...
        # Raise a warning if checksums are enabled but only if we're actually writing a compressed block
        if write_checksum:
            warnings.warn(
                (
                    "Writing compressed block with block checksums enabled. "
                    "Checksums generated by this library may be unreadable by other ASDF implementations "
                    "when block compression is also enabled."
                ),
                AsdfCompressedChecksumWarning,
            )
