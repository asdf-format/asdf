...
def test_tagged_list_base():
    ...
    assert value != value.base  # base is not a tagged list
...
def test_tagged_dict_base():
    ...
    assert value != value.base  # base is not a tagged  dict
    assert value.data == value.base  # but the data is
...
def test_tagged_string_base():
    ...
    # Note this is weird it needs the not rather than != because the base is a string
    assert not (value == value.base)  # base is not a tagged string  # noqa: SIM201
    assert value.data == value.base  # but the data is
