...
class AsdfFile:
    ...
    def __init__(
        ...
        self._blocks = block_manager.BlockManager(
            self, copy_arrays=copy_arrays, lazy_load=lazy_load, readonly=_readonly
        )
        self._uri = None
        if tree is None:
        ...
        **kwargs,
        ...
        with config_context() as config:
            ...
            try:
                ...
                if not block_manager.calculate_updated_layout(
                    self.blocks, serialized_tree_size, pad_blocks, fd.block_size
                ):
                    # If we don't have any blocks that are being reused, just
                    # write out in a serial fashion.
                    self._serial_write(fd, pad_blocks, include_block_index)
