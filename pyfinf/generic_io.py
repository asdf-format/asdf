    ...
    def read_until(self, delimiter, delimiter_name=None, include=True):
        ...
        while True:
            ...
            if index is not None:
                ...
                block = block[:index]
                buff.write(block)
                self._fd.seek(cursor + buff.tell(), os.SEEK_SET)
        ...
        while True:
            ...
            if index is not None:
                if include:
                    ...
                    index = index.start()
                self._fd.seek(-(len(block) - index), os.SEEK_CUR)
                return True
...
class InputStream(GenericFile):
    ...
    """
    def __init__(self, fd, uri=None):
        super(InputStream, self).__init__(fd, 'r', uri=uri)
        self._fd = fd
        self._buffer = b''
...
class OutputStream(GenericFile):
    ...
    """
    def __init__(self, fd, uri=None):
        super(OutputStream, self).__init__(fd, 'w', uri=uri)
        self._fd = fd
...
def get_file(init, mode='r', uri=None):
    ...
    if isinstance(init, (GenericFile, GenericWrapper)):
        ...
        return RealFile(open(init, mode + 'b'), mode, close=True, uri=uri)
