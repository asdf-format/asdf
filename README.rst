...
By default, uncompressed data blocks are not memory mapped.
Memory mapping can be enabled for efficient access by using
the ``memmap`` option of `open` when reading:
